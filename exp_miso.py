...
class MISO(Experiment):
    ...
    def __init__(self, args, physical_nodes):
        ...
        self.gpu_mig_pending = {} # contains (new_partition, new_jobs, new_code) 
        ...
        self.avail_gpus = set(self.gpu_states[:]) # constantly update it
        self.tc = 'miso'
    ...
    def start_mps_prof(self, gpu, job_new, migration, run_log):
        ...
        for job in self.gpu_mig_pending[1]: # job in new_jobs
            if job == job_new:
        ...
        print(f'started MPS profiling for new job {job_new} on GPU {gpu.index}, existing jobs {self.gpu_mig_pending[1]}', file=run_log, flush=True)
    ...
    def check_mps_finish(self, run_log, migration, mps_time=30):
        ...
        for gpu in self.gpu_states:
            ...
            print(f'MPS done, start MIG mode on GPU {gpu.index}, jobs {new_jobs}, partition {new_partition}')
            time.sleep(1)
            ...
            for job in new_jobs[:]:
                if job in self.gpu_mps_jobs[gpu.index]:
                    ...
                    self.mps_compl_batch[job] += num_iters[mapped_jobid] - self.mps_start_batch[job]
            if 'idle' in new_jobs:
                print(f'Some jobs finished during MPS, new job list is {new_jobs}')
    ...
    def try_schedule(self, job, gpu_list, migration, run_log):
        ...
        if len(sorted_gpus) > 0:
            ...
            self.gpu_mig_pending[gpu_sched] = (new_partition, new_jobs, new_code)
            sched_done = True
