...
#def padding_model_norm(mean, var,
#                        input_shape=(3,7,1),
#                        start_neurons=8, 
#                        num_outputs=1, 
#                        activation='relu'):
...
#    inputs = layers.Input(shape=input_shape)
#
#    inputs = layers.Lambda(lambda x,mu,theta: (x-mu)/math.sqrt(theta),
#                                           arguments={'mu':mean,'theta':var})(inputs)
##    pdb.set_trace() 
#    # zero pad the input
#    inputs_pad = layers.ZeroPadding2D(padding=((0,1),(0,1)))(inputs) # (4,8,1)
#
#    encoder0_pool, encoder0 = encoder_blk(inputs_pad, start_neurons, activation=activation) # (2,4,8)
#    encoder1_pool, encoder1 = encoder_blk(encoder0_pool, start_neurons*2, activation=activation) # (1,2,16)
#    center = conv_blk(encoder1_pool, start_neurons*8) # (1,2,64)
#    decoder1 = decoder_blk(center, encoder1, start_neurons*2) # (2,4,16)
#    decoder0 = decoder_blk(decoder1, encoder0, start_neurons, activation=activation) # (4,8,8)
#    outputs = layers.Conv2D(num_outputs, (1, 1), padding='same',
#                                activation='linear', name='output_layer')(decoder0) # (4,8,1)
#
#    # remove the padded data
