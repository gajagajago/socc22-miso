...
#parser.add_argument('--partition', metavar='par', type=str, help='thread partition percentage')

args = parser.parse_args()

#os.environ['CUDA_MPS_ENABLE_PER_CTX_DEVICE_MULTIPROCESSOR_PARTITIONING'] = 'true' 
os.environ['CUDA_MPS_ACTIVE_THREAD_PERCENTAGE'] = '70' 
...
class RecordBatch(keras.callbacks.Callback):
    def __init__(self):
        ...
        self.curr_epoch = 0
        self.epoch_time = {}
        self.start_meas = False # whether the power measurement has started
        self.start_time = 0
    ...
    def on_train_batch_end(self, batch, logs=None):
        ...
        if not self.start_meas:
            self.start_meas = True
            # start process
            self.start_time = time.time()
#            cmd = f'./power_meas.sh {model_name} {gpu_type}_{batch_size}'
