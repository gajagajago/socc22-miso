...
def end_mps(node, gpu): # TODO: need to figure out a way to terminate MPS on specific GPU
...
def save_jobs(node, job_list, runtime, run_log):
    ...
    if 1 in finish_status:
        print(f'checkpoing {job} is invalid due to finish', file=run_log, flush=True) #TODO: see if something can be done for jobs that are near finishing
...
def thread_func(runtime, run_log, mode='full'): # this is an instance of the Experiment class 
    ...
    sock.listen(5) 
#    global completion, gpu_states, emptied_gpu, job_exe, pid_dict
    # job_exe: {jobid: (gpuid, job_index)}
